...
| SENSOR_INFO              | Store the sensor serial number and sensor activation time (Unix seconds) with each uploaded entry as `sensorInfo: { serialNumber, activationTimeEpoch }`. Useful to track sensor sessions and correlate readings with a specific sensor.          | true                                     |          |
